...
const RenderCounter = ({ componentName, children, orientation = 'vertical' }: RenderCounterProps) => {
  ...
  return (
    <div className="mb-6">
      <Card ref={cardRef} className={cn(
        "border-2 duration-1000 border-dashed border-gray-200",
        {'flex flex-row items-center': orientation === 'horizontal'},
        )}>
        <CardHeader className="pb-2 p-4">
          <CardTitle className="text-lg flex items-center gap-2">
            <Badge 
              className="h-6 w-6 flex items-center justify-center rounded-full bg-red-500 text-white font-bold"
            >
              {renderCount.current}
            </Badge>
            {componentName}
